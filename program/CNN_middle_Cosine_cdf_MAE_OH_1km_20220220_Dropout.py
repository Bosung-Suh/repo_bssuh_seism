...
modelname = 'dist_cnn_middle_CD_OH_20220217_lr0_001_Dropout'
...
# 7.calculate model's accuracy(MAE)
#@tf.function      
...
def evaluate(model, images, labels):
    logits = model(images, training=True)    #training은 아니지만 시험삼아 한번 ON?
...
for epoch in range(training_epochs):    # 1 training epoch가 1 cycle
    ...
    for images, labels in traindataset:    # training epoch 내에서 1 batch size가 1 cycle
        ...
        loss = loss_fn(model, images, labels)
        mae_train = evaluate(model, images, labels)
        sum_loss += loss
        sum_train_mae += mae_train
        train_step += 1
    avg_loss = sum_loss / train_step    #평균 계산
