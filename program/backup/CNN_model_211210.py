...
## Save pickle
""" with open('eqdata.pkl','wb') as fh1:
    pickle.dump(dataload,fh1) #여기 아래에 quit()으로 일단 pkl 파일 작성
quit() """
## Load pickle
with open('eqdata.pkl','rb') as fh1:
...
for i in tqdm.tqdm(npylist):
    ...
    labelround = float(round(labelfloat))
...
f.write("Learning Finished! It took {modeltime}.")
f.close()
print("Learning Finished! Learning time: {modeltime}")
