...
traindata, testdata, trainlabel, testlabel = train_test_split(npys, labels, test_size=0.2, shuffle=True, random_state=42)   ######뒤로
#traindata, valdata, trainlabel, vallabel = train_test_split(restdata, restlabel, test_size=0.25, shuffle=True, random_state=42)
# train 60861(0.8), test 20288(0.2), val 20288(0.2)
...
def loss_fn(model, images, labels):    #labels=정답
    ...
    return loss     #####categorical_crossentropy  >>>maximum_likelihood?
...
def evaluate(model, images, labels):
    ...
    correct_prediction = tf.equal(tf.argmax(logits, 1), tf.argmax(labels, 1))    #정답(label)과 계산값(logit)을 비교   ####거리를 고려할 수 있게 바꾸기
