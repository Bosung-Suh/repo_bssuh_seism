...
modelname = 'dist_cnn_deep_CD_OH_20220217_lr0_0001'
...
def create_model():    #함수 형태(sequential)로 네트워크 구성
    ...
    model.add(keras.layers.MaxPool2D(padding='SAME'))
    #Fifth Conv2d+Maxpool
    model.add(keras.layers.Conv2D(filters=512, kernel_size=(1,4), activation=tf.nn.relu, padding='SAME'))
    model.add(keras.layers.MaxPool2D(padding='SAME'))
    #Sixth Conv2d+Maxpool
    model.add(keras.layers.Conv2D(filters=1024, kernel_size=(1,4), activation=tf.nn.relu, padding='SAME'))
    model.add(keras.layers.MaxPool2D(padding='SAME'))
