...
modelname = 'dist_cnn_cosine_distance_100m_20220116'
...
plt.show()
plt.savefig(r'/home/bssuh/repo_bssuh_seism/program/Test/plot/one_hot_100m.png',facecolor='#eeeeee')
...
def create_model():    #함수 형태(sequential)로 네트워크 구성
    ...
    model.add(keras.layers.Dense(1001))    #layer를 순서대로 하나씩 추가
...
def evaluate(model, images, labels):
    ...
    error_p = 2*(1-stats.norm.cdf(errordistance_float,loc=0,scale=20))    #0.1km 단위에서 표준편차 20=약 2km?
...
for epoch in range(training_epochs):    # 1 training epoch가 1 cycle
    ...
    for images, labels in traindataset:    # training epoch 내에서 1 batch size가 1 cycle
        ...
        loss = loss_fn(model, images, labels)
        acc, mae_train = evaluate(model, images, labels)    #학습 중간에 loss, acc 계산
        sum_loss += loss
    ...
    for images, labels in testdataset:      # 1 epoch가 끝나면 test dataset을 넣고 모델 검증 
        acc, mae_test = evaluate(model, images, labels)        
        sum_test_acc += acc
    ...
    epochresult = f"Epoch: {epoch + 1} loss = {avg_loss:.8f} train accuracy = {avg_train_acc:.4f} test accuracy = {avg_test_acc:.4f} train MAE = {avg_train_mae:.4f}km test MAE = {avg_test_mae:.4f}km"
